...
const weather_1 = require("../db/services/weather");
class WeatherController {
    constructor() {
        this.weatherService = new weather_1.WeatherService();
    }
    /**
     * Test endpoint to verify API key configuration
     * GET /api/weather/test
     */
    async testApiKey(request, reply) {
        try {
     ...
     */
    async getCurrentWeather(request, reply) {
        try {
            const { lat, lon, exclude, units, lang } = request.query;
     ...
     */
    async getWeatherForTimestamp(request, reply) {
        try {
            const { lat, lon, dt, units, lang } = request.query;
            // Validate required parameters
            if (!lat || !lon || !dt) {
                return reply.status(400).send({
                    success: false,
     ...
     */
    async getDailyAggregation(request, reply) {
        try {
            const { lat, lon, start, end, units, lang } = request.query;
            // Validate required parameters
     ...
     */
    async getWeatherOverview(request, reply) {
        try {
            const { lat, lon, units, lang } = request.query;
            // Validate required parameters
            if (!lat || !lon) {
                return reply.status(400).send({
                    success: false,
     ...
     */
    async getDemoWeather(request, reply) {
        try {
