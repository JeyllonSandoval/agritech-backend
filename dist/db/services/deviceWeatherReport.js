...
class DeviceWeatherReportService {
     ...
     */
    static toTimestampValueMap(input) {
        if (!input)
            return null;
        const candidate = (obj) => {
            if (!obj)
            ...
            if (obj.data && typeof obj.data === 'object') {
                return Object.fromEntries(Object.entries(obj.data).map(([ts, v]) => [String(ts), Number(v.value ?? v)]));
            }
            // Caso 2: { list: { ts: val } }
            if (obj.list && typeof obj.list === 'object') {
                return Object.fromEntries(Object.entries(obj.list).map(([ts, v]) => [String(ts), Number(v.value ?? v)]));
            }
            // Caso 3: Objeto plano { ts: val }
            if (typeof obj === 'object') {
                const keys = Object.keys(obj);
                if (keys.length > 0 && keys.every(k => /^(\d+)$/.test(k))) {
                    return Object.fromEntries(keys.map(k => [k, Number(obj[k])]));
                }
            }
            return null;
        };
        ...
        map = candidate(input.relative) || candidate(input.absolute) || candidate(input.primary);
        return map;
    }
    /**
     * Normaliza el bloque histórico para los gráficos del PDF: { temperature, humidity, pressure, soilMoisture }
     */
    static normalizeHistoricalForCharts(processed, original) {
        if (!processed)
            return null;
        const normalized = {};
        // Temperatura
        ...
        if (tempMap)
            normalized.temperature = { unit: processed.temperature?.unit || '°C', data: tempMap };
        // Humedad del aire
        ...
        if (humMap)
            normalized.humidity = { unit: processed.humidity?.unit || '%', data: humMap };
        ...
        if (pressureMap)
            normalized.pressure = { unit: 'hPa', data: pressureMap };
        ...
        if (soilMap)
            normalized.soilMoisture = { unit: '%', data: soilMap };
     ...
     */
    static hasRealtimePayload(realtimeData) {
        if (!realtimeData)
     ...
     */
    static hasHistoricalPayload(processedHistoricalData) {
   ...
   */
    static async generateDeviceReport(deviceId, userId, includeHistory = false, historyRange) {
        try {
            ...
            let originalHistoricalDataForNormalization = null;
            let timeRange = null;
        ...
        try {
            ...
            for (const device of groupDevices) {
                try {
                    ...
                    if (hasRealtime)
                        groupDiagnostic.devicesWithRealtimeData++;
                    // Clasificar
                    const summaryItem = { deviceId: device.DeviceID, deviceName: device.DeviceName };
                    if (hasHistory && hasRealtime)
                        groupDiagnostic.devicesBoth.push(summaryItem);
                    else if (hasRealtime && !hasHistory)
                        groupDiagnostic.devicesRealtimeOnly.push(summaryItem);
                    else if (!hasRealtime && hasHistory)
                        groupDiagnostic.devicesHistoryOnly.push(summaryItem);
                    else
                        groupDiagnostic.devicesNoData.push(summaryItem);
