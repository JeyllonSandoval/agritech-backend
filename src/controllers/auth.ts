...
export const resendVerificationEmail = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const { Email } = req.body as { Email: string };
...
export const validateResetToken = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        const { token } = req.params as { token: string };
